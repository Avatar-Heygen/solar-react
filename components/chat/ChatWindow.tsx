...
export default function ChatWindow({ lead }: { lead: any }) {
    ...
    return (
        <div className="flex-1 flex flex-col bg-slate-900/50 h-full">
            {/* Chat Header */}
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-950">
                <form onSubmit={handleSend} className="flex gap-2">
                    <input
                        ...
                        disabled={sending}
                        className="flex-1 bg-slate-900 border border-slate-800 rounded-full px-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50"
                    />
                    <button
                        type="submit"
                        disabled={sending}
                        className="bg-blue-600 hover:bg-blue-700 text-white rounded-full p-2 px-4 text-sm font-medium transition-colors disabled:opacity-50"
                    >
                        {sending ? '...' : 'Envoyer'}
                    </button>
                </form>
